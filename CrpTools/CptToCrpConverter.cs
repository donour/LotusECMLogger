...
namespace LotusECMLogger
{
    ...
    public class CptToCrpConverter
    {
        ...
        private static byte[] CreateEncryptedEcuData(byte[] binData)
        {
            ...
            byte[] ecuId = Encoding.ASCII.GetBytes("ECU T6".PadRight(32));
            ...
            // Calculate total buffer size (8-byte salt + 4-byte size + plain data)
            // Must be multiple of 8 for XTEA encryption
            ...
            // Create plain data buffer with proper alignment
        ...
        private static byte[] CreateTocChunk(string fileName, string description)
        {
            ...
            string paddedFileName = fileName.Length > ENTRY_NAME_SIZE
                ? fileName.Substring(0, ENTRY_NAME_SIZE)
                : fileName.PadRight(ENTRY_NAME_SIZE);
            byte[] fileNameBytes = Encoding.ASCII.GetBytes(paddedFileName);
            Array.Copy(fileNameBytes, 0, toc, fileListOffset + 4, Math.Min(fileNameBytes.Length, ENTRY_NAME_SIZE));

            // Description list: index (2) + description
            WriteInt32LE(toc, descListOffset, 2);
            string paddedDesc = description.Length > ENTRY_NAME_SIZE
                ? description.Substring(0, ENTRY_NAME_SIZE)
                : description.PadRight(ENTRY_NAME_SIZE);
            byte[] descBytes = Encoding.ASCII.GetBytes(paddedDesc);
            Array.Copy(descBytes, 0, toc, descListOffset + 4, Math.Min(descBytes.Length, ENTRY_NAME_SIZE));
